def remainder(divisor, base, exponent):
	...
	# Find if this congruence is needed for final result calculation 
	if p in exponent_sum_terms: # if p in {p1, ..., pm} then
		print("--> ", end="") # indicate that this congruence is used in the final product
		power_congruent_product_terms.append(optimal_congruent) # and store the optimal congruent of b^p
	else:
		print("    ", end="")
